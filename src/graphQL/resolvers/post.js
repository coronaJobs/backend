...
module.exports = {
  ...
  Query: {
    getAllPosts: async (_, params, ctx) => {        
        ...
        const {text, fromDate, toDate, ownerId, fromApplicantLimit, toApplicantLimit} = params
        ...
        if (fromDate && toDate) {
          filter.where.createdAt = { [Op.between]: [DateTime.fromISO(fromDate, {zone: 'UTC'}).toISO(), DateTime.fromISO(toDate, {zone: 'UTC'}).toISO()] }
        } else if (fromDate) {
          filter.where.createdAt = { [Op.gte]: DateTime.fromISO(fromDate, {zone: 'UTC'}).toISO() }
        } else if (toDate) {
          filter.where.createdAt = { [Op.lte]: DateTime.fromISO(toDate, {zone: 'UTC'}).toISO() }
        }
        ...
        if (fromApplicantLimit && toApplicantLimit) {
          filter.where.applicantLimit = { [Op.between]: [fromApplicantLimit, toApplicantLimit] }
