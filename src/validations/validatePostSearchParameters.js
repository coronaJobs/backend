...
const validateDates = (fromDate, toDate, validatorErrors) => {
  if (fromDate) {
    if (!validator.isISO8601(fromDate, { strict: true })) {
      validatorErrors.fromDate =
        "Invalid date: check format and dates that doesn't exist";
    }
  }

  if (toDate) {
    if (!validator.isISO8601(toDate, { strict: true })) {
      validatorErrors.toDate =
        "Invalid date: check format and dates that doesn't exist";
    }
  }
...
const validateApplicantLimits = (
  ...
  if (fromApplicantLimit && toApplicantLimit) {
    if (fromApplicantLimit > toApplicantLimit) {
      validatorErrors.fromApplicantLimit =
        "fromApplicantLimit can't be greater than toApplicantLimit";
      validatorErrors.toApplicantLimit =
        "toApplicantLimit can't be lower than fromApplicantLimit";
    }
  }
};
...
const validatePostSearchParameters = async (params) => {
  ...
  validateApplicantLimits(
    params.fromApplicantLimit,
    params.toApplicantLimit,
    validatorErrors
  );
